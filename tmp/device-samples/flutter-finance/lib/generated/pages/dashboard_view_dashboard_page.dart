...
class DashboardViewDashboardPage extends StatefulWidget {
  const DashboardViewDashboardPage({super.key});
  @override
  State<DashboardViewDashboardPage> createState() => _DashboardViewDashboardPageState();
}

class _DashboardViewDashboardPageState extends State<DashboardViewDashboardPage> {
  ...
  Future<void> _loadSummary() async {
    ...
    try {
      ...
      final countRows = await client.from("transactions").select("id");
      final total = (countRows as List<dynamic>?)?.length ?? 0;
      final summary = <String, num>{"total": total};
      try {
        final rows = await client.from("transactions").select("amount");
        final sum = (rows as List<dynamic>?)?.fold<num>(0, (s, r) => s + (num.tryParse((r as Map)["amount"]?.toString() ?? "0") ?? 0)) ?? 0;
        summary["amount"] = sum;
      } catch (_) {}
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(padding: const EdgeInsets.all(16), children: [
                    Wrap(spacing: 12, runSpacing: 12, children: [
                      ...
                      _SummaryCard(label: "amount 合计", value: "${(_summary['amount'] ?? 0).toStringAsFixed(1)}", icon: Icons.trending_up, color: theme.colorScheme.tertiary),
                    ]),
                    ...
                    SizedBox(
                      ...
                      child: BarChart(BarChartData(
                        ...
                        barGroups: [
                            BarChartGroupData(x: 0, barRods: [BarChartRodData(toY: (_summary["amount"] ?? 0).toDouble())])
                        ],
