...
export function emitHarmonyGame(): string {
  return `import { gameService } from '../services/IndustryServices';

@Entry @Component struct GamePage {
  ...
  start() {
    ...
    this.timer = setInterval(() => {
      if (this.timeLeft <= 1) { this.endGame(); return; }
      this.timeLeft--;
    }, 1000);
  }
  ...
  async endGame() {
    ...
    if (this.score > 0) {
      try {
        await gameService.create({ score: this.score, played_at: new Date().toISOString() });
      } catch (_e) { /* 离线时静默 */ }
    }
  }

  moveTarget() { this.tx = Math.random() * 300; this.ty = Math.random() * 400; }
  onTap() { if (!this.playing) return; this.score++; this.moveTarget(); }

  build() {
    Column() {
      Text("点击挑战").fontSize(28).fontWeight(FontWeight.Bold).margin(20);
      Text("得分: " + this.score).fontSize(48).fontWeight(FontWeight.Bold).fontColor("#0D9488");
      Text("剩余: " + this.timeLeft + "s").fontSize(16).fontColor("#9CA3AF");
      Stack() {
        Column() {
          Text("🎯").fontSize(48).position({ x: this.tx, y: this.ty })
        }.width("100%").height(400).backgroundColor("#F0FDFA").borderRadius(16).margin(16).onClick(() => { this.onTap(); })
      };
      if (!this.playing) {
        Button("开始游戏").type(ButtonType.Capsule).onClick(() => { this.start(); }).margin(20)
      }
    }.width("100%").height("100%")
  }
}`;
...
export function emitHarmonyPayment(): string {
  return `import { paymentService } from '../services/IndustryServices';

@Entry @Component struct PaymentPage {
  @State amount: string = "";
  @State method: string = "stripe";
  ...
  build() {
    Column() {
      Text("收银台").fontSize(28).fontWeight(FontWeight.Bold).margin(20);
      Column() {
        Text("金额 (¥)").fontSize(14).fontColor("#6B7280");
      ...
      Row() {
        Button({ type: this.method === "stripe" ? ButtonType.Capsule : ButtonType.Normal }, { text: "💳 银行卡" })
          .onClick(() => { this.method = "stripe"; }).margin(4);
        Button({ type: this.method === "wechat" ? ButtonType.Capsule : ButtonType.Normal }, { text: "💚 微信" })
          .onClick(() => { this.method = "wechat"; }).margin(4);
        Button({ type: this.method === "alipay" ? ButtonType.Capsule : ButtonType.Normal }, { text: "💙 支付宝" })
          .onClick(() => { this.method = "alipay"; }).margin(4);
      }.padding(16);
      Button(this.paying ? "支付中…" : "确认支付")
        .type(ButtonType.Capsule).width("100%")
        .enabled(!this.paying && this.amount.length > 0)
        .onClick(() => { this.onPay(); })
    }.width("100%").height("100%")
  }
}`;
