...
export function emitWechatGameWxml(): string {
  return '<view class="page"><privacy-popup show="{{showPrivacy}}" bind:accept="onPrivacyAccept"/><view class="game-container"><text class="game-title">点击挑战</text><text class="game-score">得分: {{score}}</text><text class="game-timer">剩余: {{timeLeft}}s</text><view class="game-area" bindtap="onTap"><text class="game-target" style="left: {{targetX}}px; top: {{targetY}}px">🎯</text></view><button class="game-start-btn" bindtap="onStart" wx:if="{{!playing}}">开始游戏</button></view></view>';
}

export function emitWechatGameJs(): string {
  return 'const { gameService } = require("../../services/industry");\nPage({data:{showPrivacy:false,score:0,timeLeft:10,playing:false,targetX:150,targetY:200},onShow(){const a=getApp();this.setData({showPrivacy:!a.globalData.privacyAccepted})},onPrivacyAccept(){const a=getApp();a.globalData.privacyAccepted=true;wx.setStorageSync("privacy_accepted",true);this.setData({showPrivacy:false})},onStart(){this.setData({score:0,timeLeft:10,playing:true});this.moveTarget();this._timer=setInterval(()=>{const t=this.data.timeLeft-1;if(t<=0){this.endGame();return}this.setData({timeLeft:t})},1000)},onTap(){if(!this.data.playing)return;this.setData({score:this.data.score+1});this.moveTarget()},moveTarget(){const w=wx.getSystemInfoSync().windowWidth-60;const h=400;this.setData({targetX:Math.random()*w,targetY:Math.random()*h})},async endGame(){clearInterval(this._timer);this.setData({playing:false});try{if(this.data.score>0)await gameService.create({user_id:getApp().globalData.userId,score:this.data.score})}catch(e){};wx.showModal({title:"游戏结束",content:"得分: "+this.data.score,showCancel:false})}});';
}

// ─── Payment (收银台) ──────────────────────────────

export function emitWechatPaymentWxml(): string {
  return '<view class="page"><privacy-popup show="{{showPrivacy}}" bind:accept="onPrivacyAccept"/><view class="payment-card"><text class="payment-title">收银台</text><view class="payment-field"><text class="payment-label">金额 (¥)</text><input class="payment-input" type="digit" value="{{amount}}" bindinput="onAmountInput" placeholder="0.00"/></view><view class="payment-methods"><text class="payment-label">支付方式</text><view class="method-list"><block wx:for="{{methods}}" wx:key="key"><view class="method-item {{item.key===selectedMethod?\'active\':\'\'}}" bindtap="onSelectMethod" data-key="{{item.key}}"><text class="method-icon">{{item.icon}}</text><text>{{item.label}}</text></view></block></view></view><button class="payment-btn" bindtap="onPay" disabled="{{!amount||paying}}">{{paying?"支付中…":"确认支付"}}</button></view></view>';
}

export function emitWechatPaymentJs(): string {
  return 'const { paymentService } = require("../../services/industry");\nPage({data:{showPrivacy:false,amount:"",selectedMethod:"stripe",paying:false,methods:[{key:"stripe",label:"银行卡",icon:"💳"},{key:"wechat",label:"微信支付",icon:"💚"},{key:"alipay",label:"支付宝",icon:"💙"}]},onShow(){const a=getApp();this.setData({showPrivacy:!a.globalData.privacyAccepted})},onPrivacyAccept(){const a=getApp();a.globalData.privacyAccepted=true;wx.setStorageSync("privacy_accepted",true);this.setData({showPrivacy:false})},onAmountInput(e){this.setData({amount:e.detail.value})},onSelectMethod(e){this.setData({selectedMethod:e.currentTarget.dataset.key})},async onPay(){const amt=parseFloat(this.data.amount);if(!amt||amt<=0){wx.showToast({title:"请输入金额",icon:"none"});return}this.setData({paying:true});try{await paymentService.create({amount:amt,currency:"cny",method:this.data.selectedMethod,status:"pending"});wx.showToast({title:"支付成功"});this.setData({amount:"",paying:false})}catch(e){wx.showToast({title:"支付失败",icon:"error"});this.setData({paying:false})}}});';
}

// ─── Game/Payment WXSS ────────────────────────────

export function emitWechatGamePaymentWxss(): string {
  return '.game-container{display:flex;flex-direction:column;align-items:center;padding:40rpx}.game-title{font-size:40rpx;font-weight:700}.game-score{font-size:56rpx;font-weight:700;color:#0D9488;margin:20rpx 0}.game-timer{font-size:24rpx;color:#9CA3AF}.game-area{width:100%;height:500rpx;background:#F0FDFA;border-radius:16rpx;margin-top:20rpx;position:relative;overflow:hidden}.game-target{position:absolute;font-size:48rpx;transition:all 0.1s}.game-start-btn{margin-top:32rpx;width:200rpx;background:#0D9488;color:#fff;border-radius:40rpx}.payment-card{padding:40rpx}.payment-title{font-size:40rpx;font-weight:700;margin-bottom:40rpx}.payment-field{margin-bottom:32rpx}.payment-label{font-size:24rpx;color:#6B7280;margin-bottom:8rpx;display:block}.payment-input{font-size:48rpx;font-weight:700;border-bottom:2rpx solid #E5E7EB;padding:12rpx 0}.method-list{display:flex;gap:16rpx;margin-top:12rpx}.method-item{padding:16rpx 24rpx;border:2rpx solid #E5E7EB;border-radius:12rpx;text-align:center;font-size:24rpx}.method-item.active{border-color:#0D9488;background:#F0FDFA}.method-icon{font-size:36rpx;display:block;margin-bottom:4rpx}.payment-btn{width:100%;background:#0D9488;color:#fff;border-radius:12rpx;padding:24rpx;font-size:32rpx;margin-top:40rpx}.payment-btn[disabled]{opacity:0.5}';
}
