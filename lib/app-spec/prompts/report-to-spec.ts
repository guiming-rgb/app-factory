...
- screen.type: 按 App 特点智能选择。基础: tabRoot | list | detail | form | placeholder。扩展: dashboard(数据看板) | card_grid(卡片网格) | calendar(日历) | chart(图表) | kanban(看板) | onboarding(引导页) | map | chat | call | payment | iot | game | ar | medical | automotive | banking | insurance | kyc
- 判断规则: 数据统计类App→dashboard+chart, 内容浏览类→card_grid, 日程类→calendar, 项目类→kanban, 新用户→onboarding
- **禁止**把 screen.id 当作 type；**禁止**使用 main/home/list/page 等作为 type
