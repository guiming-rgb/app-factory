...
class _TransactionListPageState extends State<TransactionListPage> {
  ...
  Future<void> _load([String? search]) async {
    ...
    try {
      ...
      var q = client.from("transactions").select("*").order("created_at",ascending:false).limit(50);
          ...
          RefreshIndicator(onRefresh: () => _load(), child: ListView.builder(
            itemCount: _items.length, itemBuilder: (_, i) {
              ...
              return Card(child: ListTile(
                ...
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => TransactionDetailPage(item: item))),
              ));
            }
          )),
        ),
      ]),
      floatingActionButton: FloatingActionButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TransactionFormPage())).then((_) => _load()), child: const Icon(Icons.add)),
    );
  }
}
