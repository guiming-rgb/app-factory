...
class PhotoItemListPage extends StatefulWidget {
  const PhotoItemListPage({super.key});
  @override
  State<PhotoItemListPage> createState() => _PhotoItemListPageState();
}

class _PhotoItemListPageState extends State<PhotoItemListPage> {
  ...
  Future<void> _load([String? search]) async {
    ...
    try {
      var q = Supabase.instance.client.from("photos").select("*").order("created_at",ascending:false).limit(50);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("PhotoItem")),
          ...
          RefreshIndicator(onRefresh: () => _load(), child: ListView.builder(
            itemCount: _items.length, itemBuilder: (_, i) {
              ...
              return Card(child: ListTile(
                ...
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => PhotoItemDetailPage(item: item))),
              ));
            }
          )),
        ),
      ]),
      floatingActionButton: FloatingActionButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PhotoItemFormPage())).then((_) => _load()), child: const Icon(Icons.add)),
    );
  }
}
