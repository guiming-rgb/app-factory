...
class _HotelListPageState extends State<HotelListPage> {
  ...
  Future<void> _load([String? search]) async {
    ...
    try {
      var q = Supabase.instance.client.from("hotels").select("*").order("created_at",ascending:false).limit(50);
          ...
          RefreshIndicator(onRefresh: () => _load(), child: ListView.builder(
            itemCount: _items.length, itemBuilder: (_, i) {
              ...
              return Card(child: ListTile(
                ...
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => HotelDetailPage(item: item))),
              ));
            }
          )),
        ),
      ]),
      floatingActionButton: FloatingActionButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HotelFormPage())).then((_) => _load()), child: const Icon(Icons.add)),
    );
  }
}
